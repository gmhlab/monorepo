...
export function EquipVisual() {
  return (
    <div className="relative w-full h-full min-h-[640px] overflow-hidden rounded-xl isolate">
      ...
      <FloatingPhoto
        photo={PHOTOS[0]}
        className="top-6 left-6 w-24 h-24 sm:w-28 sm:h-28"
        delay={0.3}
        bobAmplitude={-6}
        bobDuration={6}
      />
      <FloatingPhoto
        photo={PHOTOS[1]}
        className="bottom-24 left-8 w-20 h-20 sm:w-24 sm:h-24"
        delay={0.5}
        bobAmplitude={6}
        bobDuration={7.5}
      />
      <FloatingPhoto
        photo={PHOTOS[2]}
        className="bottom-8 right-8 w-24 h-24 sm:w-28 sm:h-28"
        delay={0.45}
        bobAmplitude={-7}
        bobDuration={8}
      />
      ...
      <div className="absolute top-6 right-6 flex flex-col gap-2 items-end">
        {STATS.map((stat, i) => (
          <motion.div
            key={stat.label}
            className="backdrop-blur-md bg-white/10 ring-1 ring-white/25 rounded-2xl pl-3 pr-4 py-2.5 shadow-xl flex items-center gap-3 group"
            initial={{ opacity: 0, x: 24 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.7 + i * 0.15, duration: 0.55, ease: "easeOut" }}
            whileHover={{ scale: 1.04, x: -2 }}
          >
            <div className="w-8 h-8 rounded-xl bg-[#AA9868]/20 ring-1 ring-[#AA9868]/40 flex items-center justify-center">
              <stat.icon className="w-4 h-4 text-[#F5E6BE]" />
            </div>
...
function FloatingPhoto({
  ...
  return (
    <motion.div
      className={`absolute rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/25 ${className}`}
