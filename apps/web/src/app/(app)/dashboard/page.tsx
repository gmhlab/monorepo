...
export default function DashboardPage() {
  return (
    <Stack gap="xl">
      ...
      <SectionHeader title="Recent Activity" description="Latest updates across the platform" />

      <Grid columns="auto-fit" min="20rem" gap="lg">
        <Card>
          <CardHeader>
            <CardTitle>Training Sessions</CardTitle>
            <CardDescription>Active training programs this quarter</CardDescription>
          </CardHeader>
          <CardContent>
            <Stack gap="sm">
              {["EQUIP Competency Training — Uganda", "Photovoice Workshop — Kenya", "Group IPT Supervision — Nepal"].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-md border border-border/40 p-3 text-sm">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Competency evaluations completed this week</CardDescription>
          </CardHeader>
          <CardContent>
            <Stack gap="sm">
              {[
                ...
                <div key={item.name} className="flex items-center justify-between rounded-md border border-border/40 p-3 text-sm">
                  <span>{item.name}</span>
